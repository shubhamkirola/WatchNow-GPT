  ...
  return (
    <div>
      ...
      <div className='bg-gradient-to-b from-black to-white relative'>
        ...
        <div className='w-72 p-6 absolute top-[25%] left-[40%] border rounded bg-black bg-opacity-70'>
          ...
          <form className='flex flex-col' onSubmit={(e) => e.preventDefault()}>
            {!signIn && (<input
              ...
              className='py-2 pl-2 mt-4 border rounded bg-slate-800 text-white'>
            </input>)}
            <input 
              ref={email} 
              type="email" 
            ...
            <input 
              ...
              className='mt-4 py-2 pl-2 border rounded  bg-slate-800 text-white' >
            </input>
            <button 
              className='h-1/4 mt-4 border rounded bg-red-700 py-2 font-bold' 
              onClick={handlesignIn}>{signIn ? "Sign In" : "Sign Up"}</button>
          </form>
          <p className='text-white'>{errMessage} </p>
          <h3 
            className='mt-4 text-white cursor-pointer font-bold' 
